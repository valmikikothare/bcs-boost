...
class MemberController extends Controller
{
    ...
    public function mySessions(Request $request)
    {
        ...
        $leadSlots = $leadQuery->with([
            ...
            $sessionLead       = $slot->sessionLeads->firstWhere('user_id', $userId);
            $slot->role        = 'Leader';
            $slot->status      = $sessionLead->status ?? 0;
            $slot->lead_status = $sessionLead->status ?? 0;
            $slot->description = $sessionLead->description ?? '';
            // Add flag for blade
