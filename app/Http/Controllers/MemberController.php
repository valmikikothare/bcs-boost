...
class MemberController extends Controller
{
    ...
    public function availableslots(Request $request)
    {
        ...
        $slots = Slot::with([
            ...
            ->where(function ($q) use ($userId) {
                $q->where('status', 0) // available slots
                    ->orWhereHas('sessionLeads', function ($sub) use ($userId) {
                        $sub->where('user_id', $userId)->where('status', 1); // confirmed slots for this user
                    });
            })
            ->get();
    ...
    public function book_session_list(Request $request)
    {
        ...
        // Available Slots (user hasn't booked yet)
        ...
        $slots = $query->with('sessionLeads.user')->get();

        // ✅ Confirmed bookings for this user
        $confirmedBookings = BookingHistory::with(['slot', 'slot.sessionLeads.user'])
            ->where('user_id', $userId)
            ->where('status', 1)
        ...
        // ✅ Slots where user is lead and approved
        $leadSlots = Slot::whereHas('sessionLeads', function ($q) use ($userId) {
        ...
        return view('frontend.members.bookinglist', compact('slots', 'confirmedBookings', 'leadSlots'));
    }
