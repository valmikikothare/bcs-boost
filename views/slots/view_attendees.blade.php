    ...
    <div class="content">
        <div class="container-fluid">
            <div class="d-flex justify-content-between bg-dark px-4 py-3">
                ...
                <div>
                    <a class="btn btn-success" href="{{ route('export.booking.history.csv') }}">Export CSV</a>
                                ...
                                @if (count($attendees_list->bookinghistory) > 0)
                                    @foreach ($attendees_list->bookinghistory as $attendees)
                                        <tr id="slotRow_{{ $attendees->id }}">
                                            <td>{{ ucfirst($attendees_list->name ?? 'N/A') }}</td>

                                            <td>{{ ucfirst(string: optional($attendees->user)->name ?? 'N/A') }}</td>

                                            <td>{{ optional($attendees->user)->email }}</td>
